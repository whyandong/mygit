...
def pytest_runtest_makereport(item, call):
    ...
    if constant.unionTest_config is not None:
        if constant.unionTest_config.get('transform', None):
            ...
            elif constant.unionTest_config['transform'].get('style') == 'simple':
    ...
    if report.when == 'teardown':
        ...
        write_result2file()
        if constant.unionTest_config['transform']['mode'] == 'immediately':  # 立即上传
...
def pytest_collection_modifyitems(config, items):
    ...
    [delete_list.append(item['item']) for module in delete_module for item in items_info_list if
     item['file'].startswith(f'{module}/')]

    [delete_list.append(item['item']) for file in delete_file for item in items_info_list if
     item['file'] == file and item['item'] not in delete_list]

    [delete_list.append(item['item']) for function in delete_function for item in items_info_list if
     item['nodeid'] == function and item['item'] not in delete_list]
