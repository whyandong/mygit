import allure
import pytest
from frame.constant import delete_app
...
class Test_linglong:
    ...
    @pytest.mark.parametrize('expected,url,headers,data,casename', post_read_json(delete_app))
    def test_task_list(self,expected,url,headers,data,casename):
        ...
        print(progreess_path)
        headers['X-Token'] = get_login()
        result = requests.delete(url=progreess_path,headers=headers,data=data).json()
        print(result)
...
if __name__ == '__main__':
    pytest.main(['-s', 'test_delete_app.py'])