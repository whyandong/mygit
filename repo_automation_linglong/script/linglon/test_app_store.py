import allure
import pytest
from frame.constant import app_store
...
class Test_linglong:
    ...
    @pytest.mark.parametrize('expected,url,headers,data,casename', post_read_json(app_store))
    def test_task_list(self,expected,url,headers,data,casename):
        ...
        result = requests.post(url=progreess_path,data=data).json()
        print(result)
...
if __name__ == '__main__':
    pytest.main(['-s', 'test_app_store.py'])