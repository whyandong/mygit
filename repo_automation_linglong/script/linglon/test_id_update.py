import allure
import pytest
from frame.constant import idupdate
...
class Test_linglong:
    ...
    @pytest.mark.parametrize('expected,url,headers,data,casename', post_read_json(idupdate))
    def test_task_list(self,expected,url,headers,data,casename):
        ...
        result = requests.put(url=progreess_path,headers=headers,data=data).json()
        print(result)
...
if __name__ == '__main__':
    pytest.main(['-s', 'test_id_update.py'])