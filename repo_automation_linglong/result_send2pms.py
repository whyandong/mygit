...
def send_all_result2pms():
    ...
    new_dict = suite_results(result_dict)
    for item in new_dict:
        send_result2pms(item, new_dict[item])
    logging.info(f'{"=*" * 30}所有测试结果上传到pms完毕{"=*" * 30}')

def suite_results(result_dict):
    new_dict = dict()
    for item in result_dict:
        test_case = item.split('::')[0].split('/')[-1][:-3]
        result = result_dict[item]['result']
        if new_dict.get(test_case) and new_dict[test_case]['result'] == 'fail':
            continue 
        else:
            new_dict[test_case] = result_dict[item]
    return new_dict
...
def write_result2file():
    ...
    if constant.write_result_temp_dir is not None:
        ...
        simple_result_dict = {key: {"result": result_dict[key]["result"], "longrepr": result_dict[key]["longrepr"]} for
                              key in result_dict}
        with open(file_path, 'w', encoding='utf8') as f:
...
def write_total_run_script2file(items):
    ...
    if constant.write_result_temp_dir is not None:
        file_path = os.path.join(constant.write_result_temp_dir, file_name)
        detailed_run_case_file_path = os.path.join(constant.write_result_temp_dir, detailed_run_case_file)
