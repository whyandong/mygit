...
def install_ltp(option,number):
    ...
    if option == "-f":
        if not os.path.exists("sys") and not os.path.exists("bist"):
            os.system("mkdir -v sys")
...
struct ustat
    {
        ...
        '''
        print ("*"*100)
        cpfile = "cp /usr/include/%s-linux-gnu/sys/ustat.h  %s/sys/" %(name,testpath)
        print (cpfile)
        os.system(cpfile)

        resys = "sed -i  '/^extern/ s/^\(.*\)$/\/\/\1/' /usr/include/%s-linux-gnu/sys/ustat.h " %name
        command(resys)


        cpfile = "cp /usr/include/%s-linux-gnu/bits/ustat.h  %s/" %(name,testpath)
        print (cpfile)
        os.system(cpfile)
        rebits = "sed -i '/^struct/,+6 s/^\(.*\)$/\/\/\1/' /usr/include/%s-linux-gnu/bits/ustat.h" %name
        command(rebits)

        #切换目录,使用 os.system("目录") 不能切换目录,程序执行目录
        os.chdir("ltp")

        cmd = "sudo git checkout master"
        command(cmd)
        ...
        cmd = "mv sys/ustat.h /usr/include/%s-linux-gnu/sys/ustat.h" %name
        command(cmd)
        cmd = "mv ustat.h /usr/include/%s-linux-gnu/bits/ustat.h" %name
        command(cmd)
...
def sed_email(path,ltpname,whiteList,failList,unknownList):
    ...
    # 收件人
    receivers = [
        "guobin@uniontech.com","zhengwei@uniontech.com","chenna@uniontech.com","xiongzhen@uniontech.com"
        ]
