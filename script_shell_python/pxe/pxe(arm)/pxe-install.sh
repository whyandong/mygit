...
n=`apt-cache policy debian-installer-10-netboot-arm64 |grep '已安装' |cut -c 16-18`
if [ $n = '无' ];then
	apt install  debian-installer-10-netboot-arm64 -y 
fi
...
chmod 777 *.iso
mount *.iso /mnt/
cp -r /mnt/* /var/nfs/arm64/
cp -r /mnt/.disk/ /var/nfs/arm64/
chown -R root:root /var/nfs/
find /var/nfs/arm64/ -type d -exec chmod 755 {} \;
...
mkdir /var/tftp
cp -r /usr/lib/debian-installer/images/10/arm64/text/debian-installer /var/tftp
chmod 777 /var/tftp/debian-installer/arm64/grub/grub.cfg
cat grub.txt  > /var/tftp/debian-installer/arm64/grub/grub.cfg
cp -r /mnt/live /var/tftp/debian-installer/arm64
cp /var/tftp/debian-installer/arm64/grubaa64.efi /var/tftp/
group='dnsmasq'
