...
n=`apt-cache policy debian-installer-10-netboot-amd64 |grep '已安装' |cut -c 16-18`
if [ $n = '无' ];then
	apt install  debian-installer-10-netboot-amd64 -y 
fi
...
find /var/nfs/amd64/ -type d -exec chmod 755 {} \;
...
chmod 777 /var/tftp/debian-installer/amd64/grub/grub.cfg
cat grub.txt  > /var/tftp/debian-installer/amd64/grub/grub.cfg
cp -r /mnt/live /var/tftp/debian-installer/amd64
cp /var/tftp/debian-installer/amd64/grubx64.efi /var/tftp/
group='dnsmasq'
