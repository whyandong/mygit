...
class HTMLReport:
    ...
    def _appendrow(self, outcome, report):
        info_dict = {'错误':'error','失败':'failed','跳过':'Skipped','重试':'rerun','预期失败':'xfailed','非预期通过':'xpassed','通过':'passed'}
        ...
        if result.row_table is not None:
            ...
            tbody = html.tbody(
                result.row_table,
                class_="{} results-table-row".format(info_dict[result.outcome].lower()),
            )
    ...
    def append_passed(self, report):
        if report.when == "call":
            ...
            #     self.xpassed += 1
            #     self._appendrow("失败", report)
            # else:
            self.passed += 1
            self._appendrow("通过", report)
    ...
    def append_failed(self, report):
        if getattr(report, "when", None) == "call":
            ...
            #     self.xpassed += 1
            #     self._appendrow("非预期通过", report)
            # else:
            self.failed += 1
            self._appendrow("失败", report)
        else:
            self.errors += 1
            self._appendrow("错误", report)

    def append_skipped(self, report):
        # if hasattr(report, "wasxfail"):
        #     self.xfailed += 1
        #     self._appendrow("预期失败", report)
        # else:
        self.skipped += 1
        self._appendrow("跳过", report)

    def append_other(self, report):
        # For now, the only "other" the plugin give support is rerun
        self.rerun += 1
        self._appendrow("重试", report)
    ...
    def _generate_report(self, session):
        ...
        outcomes = [
            Outcome("passed", self.passed,label='通过用例'),
            Outcome("skipped", self.skipped,label='跳过用例'),
            Outcome("failed", self.failed,label='失败用例'),
            Outcome("error", self.errors, label="错误用例"),
        ]

        if self.rerun is not None:
            outcomes.append(Outcome("rerun", self.rerun,label='失败重试用例'))
