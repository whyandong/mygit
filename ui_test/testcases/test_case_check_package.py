...
class Test:
    ...
    @pytest.mark.parametrize('data', [data_test[1]])
    def test_check_package(self, data):
        """系统软件包状态检查"""
        self.test_check_package.__func__.__doc__ = data['casename']
        LOG.info('输入参数：用例名:%s,参数:%s' % (
            data['casename'],
            data['parms'],
        ))
        response = select_file(self.test_check_package.__name__, data['parms'])
