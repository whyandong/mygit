...
class Test:
    ...
    @pytest.mark.parametrize('data', [data_test[4]])
    def test_check_system_cmd(self, data):
        """终端指令-文件系统相关"""
        self.test_check_system_cmd.__func__.__doc__ = data['casename']
        LOG.info('输入参数：用例名:%s,参数:%s' % (
            data['casename'],
            data['parms'],
        ))
        response = select_file(self.test_check_system_cmd.__name__, data['parms'])
